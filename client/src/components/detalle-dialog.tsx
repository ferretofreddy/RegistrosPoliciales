...
export default function DetalleDialog({
  ...
  switch (tipo) {
      ...
      contenido = (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-500">Telefonos</p>
              <div className="flex flex-wrap gap-1 mt-1">
                {persona.telefonos?.map((telefono, index) => (
                  <Badge key={index} variant="outline">{telefono}</Badge>
                ))}
                {!persona.telefonos?.length && <span className="text-gray-500">Sin teléfonos</span>}
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                ...
                {!persona.domicilios?.length && <span className="text-gray-500">Sin domicilios</span>}
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                ...
                {!persona.alias?.length && <span className="text-gray-500">Sin alias</span>}
              </div>
            </div>
          </div>
    ...
    case "ubicacion":
      ...
      color = "bg-red-100";
      titulo = `Ubicación #${ubicacion.id}`;
      contenido = (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <p className="text-sm font-semibold text-gray-500">Tipo</p>
              <p>{ubicacion.tipo || "No especificado"}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p>{ubicacion.latitud && ubicacion.longitud ? `${ubicacion.latitud}, ${ubicacion.longitud}` : "No disponible"}</p>
            </div>
          </div>
