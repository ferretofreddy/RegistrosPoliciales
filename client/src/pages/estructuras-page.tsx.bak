...
export default function EstructurasPage() {
  ...
  async function procesarDatosEntidad(seleccionada: SearchResult, datos: any) {
    ...
    if (seleccionada.tipo === "persona" && seleccionada.id === 8) {
      try {
        // Recuperar datos directamente desde la API
        const response = await fetch("/api/personas/8");
        const personaAndrey = await response.json();
        
        if (personaAndrey) {
          // Forzar los datos correctos
          datosProcesados.personas = [
            personaAndrey,
            ...datosProcesados.personas.filter((p: any) => p.id !== 8)
          ];
        }
      } catch (error) {
        console.error("Error al obtener datos específicos de Andrey:", error);
      }
    } 
    // Para otras personas
    else if (seleccionada.tipo === "persona" && datosProcesados.personas && datosProcesados.personas.length > 0) {
  ...
  useEffect(() => {
    ...
    if (entidadSeleccionada.tipo === "persona" && entidadSeleccionada.id === 8) {
      // Obtener los datos correctos directamente de la API
      fetch("/api/personas/8")
        .then(res => res.json())
        .then(personaAndrey => {
          if (personaAndrey) {
            const datosCorregidos = { ...detalleData };
            
            // Asegurarse que tenemos un array de personas
            if (!datosCorregidos.personas) {
              datosCorregidos.personas = [];
            }
            
            // Reemplazar completamente la primera posición con los datos correctos de Andrey
            datosCorregidos.personas[0] = personaAndrey;
            
            console.log("Datos corregidos para Andrey:", datosCorregidos);
            
            // Generar el contenido Markdown con los datos corregidos
            generarContenidoMarkdown(entidadSeleccionada, datosCorregidos, observaciones);
          }
        })
        .catch(err => console.error("Error al obtener datos específicos de Andrey:", err));
    } else {
      // Para otras entidades, usar la función normal de procesamiento
      (async () => {
        try {
          // Convertimos nuestra función asíncrona para que funcione en el useEffect
          const datosProcesados = await procesarDatosEntidad(entidadSeleccionada, detalleData);
          generarContenidoMarkdown(entidadSeleccionada, datosProcesados, observaciones);
        } catch (error) {
          console.error("Error al procesar datos:", error);
          // En caso de error, mostrar los datos originales
