      ...
      try {
        ...
        if (personaResponse.ok) {
          ...
          if (personaData.domicilios && personaData.domicilios.length > 0) {
            // Buscar ubicaciones reales para los domicilios de la persona
            try {
              const ubicacionesResponse = await fetch(`/api/relaciones/persona/${entity.id}`);
              if (ubicacionesResponse.ok) {
                const ubicacionesData = await ubicacionesResponse.json();
                if (ubicacionesData.ubicaciones && ubicacionesData.ubicaciones.length > 0) {
                  directLocations = ubicacionesData.ubicaciones.map((ubicacion: any) => ({
                    id: ubicacion.id,
                    lat: ubicacion.latitud,
                    lng: ubicacion.longitud,
                    title: "Domicilio",
                    description: `Domicilio de ${personaData.nombre}: ${ubicacion.observaciones || 'Sin observaciones'}`,
              ...
              console.error("Error obteniendo ubicaciones de persona:", error);
            }
          }
        }
      } catch (error) {
